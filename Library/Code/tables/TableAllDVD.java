...
public class TableAllDVD {
	...
	public static TableColumn returnDate = new TableColumn("Return Date");
	...
	public static void table(String name) {
		...
		String tablename = "All DVD's";
		
		Label label = new Label(tablename);
		label.setFont(new Font("Arial", 20));
		
		Database.selectAllDVD();
		
		if(i == 0) {
			table.getColumns().addAll(title, author, available, lendDate, returnDate);
			i++;
		} else {
		}
		
		title.prefWidthProperty().bind(table.widthProperty().multiply(0.2));
		author.prefWidthProperty().bind(table.widthProperty().multiply(0.2));
		...
		returnDate.prefWidthProperty().bind(table.widthProperty().multiply(0.2));
