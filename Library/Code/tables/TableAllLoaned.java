...
public class TableAllLoaned {
	...
	public static TableColumn user = new TableColumn("Username");
	...
	public static void table(String name) {
		...
		String tablename = "All loaned medias";
				
		Label label = new Label(tablename);
		label.setFont(new Font("Arial", 20));
		
		Database.selectAllLoaned();
		
		if(i == 0) {
			table.getColumns().addAll(sort, title, author, user, lendDate);
			i++;
		} else {
		}
		...
		user.prefWidthProperty().bind(table.widthProperty().multiply(0.2));
